  ...
  try {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        const { getCachedPanchang, savePanchangToCache, requestPanchangFromNative } = 
          await import('./panchangCache');

        // Step 1: Try to get from native cache (React Native mobile app)
        if (window.ReactNativeWebView) {
          const nativeData = await requestPanchangFromNative(dateStr);
          if (nativeData) {
            console.log(`Loaded panchang from native cache: ${dateStr}`);
        ...
        // Step 2: Try to get from browser cache (IndexedDB/localStorage)
        const cachedData = await getCachedPanchang(dateStr);
        if (cachedData) {
          console.log(`Loaded panchang from browser cache: ${dateStr}`);
          return cachedData;
        }
      } catch (cacheError) {
        // If cache fails, continue to server fetch
        console.warn('Cache error, falling back to server:', cacheError);
      }
    }

    // Step 3: Download from server (works in both SSR and client)
    console.log(`Downloading panchang from server: ${dateStr}`);
    
    // Use relative URL - Next.js fetch handles SSR automatically
