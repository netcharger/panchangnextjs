  ...
  try {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        // Step 2: For web view, bypass cache and load directly from URL for testing
        console.log(`🌐 Web View (Testing): Loading panchang directly from URL: ${dateStr}`);
        const url = `${API_BASE}/media/panchang_files/${dateStr}.json`;
        console.log(`Web View Fetch URL: ${url}`); // Added for clearer logging
        const response = await fetch(url, {
          cache: 'no-store', // Always fetch fresh data for web view testing
        });

        if (!response.ok) {
          console.error(`Panchangam file not found from Django: ${dateStr}.json (Status: ${response.status})`);
          return null;
        }
        ...
        // If cache fails, continue to server fetch, or if Django fetch failed for some reason
        console.warn('Web View fetch error, attempting fallback or returning null:', cacheError);
        return null; // Return null if web view fetch fails
      }
    }

    // This block now serves as the primary SSR fetch or a final fallback
    // For static files in public folder, relative URLs work in both SSR and client
    const url = `/panchang_files/${dateStr}.json`;
        // Step 3: Download from server (works in both SSR and client)
        console.log(`🌎 SSR/Fallback: Downloading panchang from server: ${dateStr} \n ${url}`);
    // Use Next.js fetch which handles SSR correctly
    const response = await fetch(url, {
      cache: 'force-cache', // Cache the response for web, but only if not in a WebView
    });
    ...
    // Save to cache for future use (only on client and if not in WebView)
