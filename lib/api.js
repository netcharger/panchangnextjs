...
const { DJANGO_BACKEND_URL, USE_STATIC_DATA } = require("./constants.js");
const { transformFlatPanchangData } = require("./dataAdapter.js");

const API_BASE = DJANGO_BACKEND_URL;
...
// Helper to fetch local JSON data
const fetchLocal = async (filename) => {
  try {
    let baseUrl = '';
    if (typeof window === 'undefined') {
      // Server-side: use the app URL or localhost
      baseUrl = process.env.NEXT_PUBLIC_WEB_APP_URL || 'http://localhost:3000';
    }
    const url = `${baseUrl}/data/${filename}`;
    console.log(`📂 Fetching static data: ${url}`);
    const response = await fetch(url);
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    const data = await response.json();
    
    // Fix image URLs in data if they point to the old Django backend
    const dataStr = JSON.stringify(data).replace(/https?:\/\/api\.dailypanchangam\.com\/media/g, '/media');
    return JSON.parse(dataStr);
  } catch (error) {
    console.error(`❌ Error fetching local data (${filename}):`, error);
    return null;
  }
};
...
if (typeof window !== 'undefined') {
  console.log('🌐 API Mode:', USE_STATIC_DATA ? 'STATIC' : 'DJANGO');
...
exports.fetchCarouselImages = async () => {
  try {
    if (USE_STATIC_DATA) {
      return await fetchLocal('carousel_images.json') || [];
    }
...
exports.fetchCategories = async () => {
  try {
    if (USE_STATIC_DATA) {
      return await fetchLocal('categories.json') || [];
    }
...
exports.fetchPostsByCategory = async (categorySlug, page = 1, pageSize = 20) => {
  try {
    if (USE_STATIC_DATA) {
      const allPosts = await fetchLocal(`posts_category_${categorySlug}.json`) || 
                       await fetchLocal('all_posts.json') || [];
      const posts = Array.isArray(allPosts) ? allPosts.filter(p => p.category_slug === categorySlug || !categorySlug) : [];
      return {
        posts: posts,
        count: posts.length,
        next: null,
    ...
    // Return structured object with pagination info
    return {
      posts: dataArray,
...
exports.fetchPostBySlug = async (slug) => {
  try {
    if (USE_STATIC_DATA) {
      const allPosts = await fetchLocal('all_posts.json');
...
// Fetch next page for category posts
exports.fetchCategoryPostsNextPage = async ({ pageParam = 1, queryKey }) => {
  const [, categorySlug] = queryKey;

  try {
    // If pageParam is a URL string
    ...
    // Use numeric page
    return await exports.fetchPostsByCategory(categorySlug, pageParam, 20);
  } catch (error) {
...
exports.fetchPanchangam = async (date, language = 'te') => {
  try {
    ...
    if (USE_STATIC_DATA) {
        ...
        const localData = await fetchLocal(`${dateStr}.json`);
        if (localData) {
            const transformed = localData.sections ? localData : transformFlatPanchangData(localData);
            return { ...transformed, _dataSource: 'Static JSON (Local)' };
        }
        
        // Try yearly total file
        const yearlyData = await fetchLocal(`${year}_total_panchangam.json`);
        if (Array.isArray(yearlyData)) {
            const dailyData = yearlyData.find(d => d.date === dateStr);
            if (dailyData) {
                const transformed = dailyData.sections ? dailyData : transformFlatPanchangData(dailyData);
                return { ...transformed, _dataSource: 'Static Yearly JSON (Local)' };
            }
        }
    }
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        const { getCachedPanchang, savePanchangToCache, requestPanchangFromNative, ensureYearlyDataCached } = await import('./panchangCache.js');

        // Step 0: Ensure yearly data is cached (Smart Caching)
        if (!window.ReactNativeWebView) {
           await ensureYearlyDataCached(year, API_BASE);
        }

        // Step 1: Try to get from native cache (React Native mobile app)
        if (window.ReactNativeWebView) {
          console.log(`🔍 FETCHING SOURCE: Checking Native Cache for ${dateStr}...`);
          ...
          if (nativeData) {
            ...
            const transformed = transformFlatPanchangData(nativeData);
            return { ...transformed, _dataSource: 'Native Cache' };
          }
        } 
        
        // Step 2: Try to get from browser storage (IndexedDB/LocalStorage)
        // This will now be instant due to ensureYearlyDataCached above
        console.log(`🔍 FETCHING SOURCE: Checking Browser Cache/IndexedDB for ${dateStr}...`);
        const cachedData = await getCachedPanchang(dateStr);
        if (cachedData) {
          console.log(`✅ Browser: Data loaded from local storage for ${dateStr}`);
          const transformed = transformFlatPanchangData(cachedData);
          if (!transformed) {
             console.error('❌ Transformation failed for cached data');
             return null;
          }
          return { ...transformed, _dataSource: 'Yearly Cache (IndexedDB)' };
        }
    ...
    try {
      ...
      if (response.ok) {
        console.log(`✅ DATA SOURCE: Django API (${djangoUrl})`);
        const data = await response.json();
        // Determine if data needs transformation (check if it has sections)
        const transformed = data.sections ? data : transformFlatPanchangData(data);
        return { ...transformed, _dataSource: `Django API (${djangoUrl})` };
      }
    } catch (e) {}

    // Final fallback: Local public folder
    const localUrl = `/panchang_files/${dateStr}.json`;
    console.log(`🔍 FETCHING SOURCE: Attempting Local Public Folder Fallback (${localUrl})...`);
    const localResponse = await fetch(localUrl, { cache: 'no-store' });
    if (localResponse.ok) {
        console.log(`✅ DATA SOURCE: Local Public Folder`);
      const localData = await localResponse.json();
      const transformed = localData.sections ? localData : transformFlatPanchangData(localData);
      return { ...transformed, _dataSource: 'Local Public Folder Fallback' };
    }
...
exports.fetchWallpaperMainCategories = async () => {
  try {
    if (USE_STATIC_DATA) {
      return await fetchLocal('wallpaper_categories.json') || [];
    }
...
exports.fetchWallpaperSubCategories = async (mainCategoryId) => {
  try {
    if (USE_STATIC_DATA) {
      return await fetchLocal(`wallpaper_subcats_${mainCategoryId}.json`) || [];
    }
...
exports.fetchMusicCategories = async () => {
  try {
    if (USE_STATIC_DATA) {
      return await fetchLocal('audio_categories.json') || [];
    }
...
exports.fetchAudioFilesByCategory = async (categorySlug) => {
  try {
    if (USE_STATIC_DATA) {
      return await fetchLocal(`audio_files_${categorySlug}.json`) || [];
    }
